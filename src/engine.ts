...
export class Anim {
  ...
  constructor(
    ...
    } else this.el.style.transform = `translateY(${-sheetPose * dsplySize + unfuckPosition[0]}px) translateX(${unfuckPosition[1]}px)`;
    
    if (cName === 'baloon') this.currDir = Math.random() >= 0.5 ? 'right' : 'left';
  ...
  movePlayer(dir: string, xyMove: number[]) {
    ...
    document.querySelector<HTMLElement>('#test').style.marginLeft =  `calc(50vw + ${-this.left}px)`
    document.querySelector<HTMLElement>('#test').style.marginTop = `calc(50vh + ${-this.top}px)`
  ...
  moveBaloon(obj: object) {
    if (this.repeat) {
      ...
      if (Math.random() >= 0.8) this.currDir = dirs[Math.round(Math.random() * 3)];
    }
  }
  ...
  goAnim() {
    if (this.moving) {
      this.currMove = this.frames[this.currDir];
      if (this.el.className.includes("player")) this.actFrame >= this.currMove.length ? this.actFrame = 0 : null
      ...
      if (this.repeat && this.actFrame == this.currMove.length) this.actFrame = 0;
