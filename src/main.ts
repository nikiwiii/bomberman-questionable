...
import {_data, _spritesheet} from './interfaces.js'
//@ts-ignore
...
let height: number
let width: number
let allSprites: _spritesheet; // tablica z animacjami
let data: _data
...
let baloons: any[] = []
let urGamerTag: string
let flameFields: number[][] = [[],[],[],[],[],[],[],[],[]]
let players = {}
let deadBaloons = []
let urStyle = ""
let spriteCount = 0
...
window.onload = () => {
  document.querySelector<HTMLDivElement>('#app')!.innerHTML = `<div id="test"><div id="sprites"></div></div>`;
  body = $("test")!;
  const input: HTMLInputElement = $('nick') as HTMLInputElement
...
const enterGamertag = async(nick: string) => {
  if (nick) {
    document.querySelector<HTMLInputElement>('#submit-btn')!.disabled = true
    const res = await fetch(`http://d3bomb.ct8.pl/webtest/verify.php?login=${nick}`)
    const verified = await res.text()
    if (verified == "ight") {
      document.querySelector<HTMLInputElement>('.form')!.style.opacity = '0'
      document.querySelector<HTMLInputElement>('#submit-btn')!.style.opacity = '0'
      await new Promise((r) => setTimeout(r, 500))
      document.querySelector<HTMLInputElement>('.form')!.style.display = 'none'
      document.querySelector<HTMLInputElement>('#submit-btn')!.style.display = 'none'
      urGamerTag = nick
      initiateGame()
    } else {
      alert("Popraw gamer-tag (tylko litery, cyfry, nie może być pusty)")
      document.querySelector<HTMLInputElement>('#submit-btn')!.disabled = false
    }
  }
}

const send = (key: string, val: any) => websocket.send(JSON.stringify({key: key, contents: val}));
...
const newPlayer = (npi: { nick: string; data: any; }) => {
...
const playerGone = (thatUser: string) => {
...
const createGameBoard = () => {
  ...
  for (let y = 0; y < height; y++) {
    ...
    const tileTypes: string[] = [elements.grassUrl!, elements.stoneUrl!, elements.brickUrl!, elements.powerUrl!];
    container.className = 'container';
    for (let x = 0; x < width; x++) {
      const el = x === powUpPose[0] && y === powUpPose[1] && x !== 0 ? 5 : gameBoard[y][x]
      const field: HTMLElement = helper.newTile(`${x},${y}`, 'field', tileTypes[el - 2], x, y, dsplySize);
...
const animate = () => {
  ...
  allSprites.baloons.forEach((e: { moving: any; goAnim: () => void }) => e.goAnim());
  allSprites.bomb.moving ? allSprites.bomb.goAnim() : null;
  if (movePlayer && controlActive) allatPlayerMoveShi()//movement
  if (exploding) {
    let booms = 0;
...
const updateGameBoard = () => {
  gameBoard.forEach((yE, y) => {
    yE.forEach((xE, x) => {
      const tileTypes: string[] = [elements.grassUrl!, elements.stoneUrl!, elements.brickUrl!];
      if (xE == 6) {
  ...
  baloons.forEach((e, i) => {
    if (allSprites.baloons[i].currDir != "dead") {
      ...
      if (!e.alive && !e.barrel) {
        ...
        if (e.barrel && e.alive) allSprites.baloons[i].moving = true
        allSprites.baloons[i].goTo(e.pos[0], e.pos[1])
        if (allSprites.baloons[i].pos[0] === allSprites.player.pos[0] && allSprites.baloons[i].pos[1] === allSprites.player.pos[1] && e.alive) killPlayer();
      }
    }
  }) 
}
...
const updatePlayers = () => {
  Object.keys(players).forEach((e) => {
    ...
    if ($(e)) $(e).innerHTML = nick+" deaths:"+players[e].deaths+" kills:"+players[e].kills
    else $("glist").innerHTML += "<div id='"+e+"' style='filter: "+players[e].color+"'>"+nick+" deaths:"+players[e].deaths+" kills:"+players[e].kills+"</div>"
...
const handleBinds = (e: KeyboardEvent) => {
  ...
  if (moveBinds.includes(prKey)) {//wasd
    ...
    movePlayer = true;
  } else if (prKey == 90 && !exploding && !allSprites.bomb.moving) explode(allSprites.player.pos[0], allSprites.player.pos[1]);
};
...
const allatPlayerMoveShi = () => {
  ...
  allSprites.baloons.forEach((e: { pos: any[] }) => {
    if (e.pos[0] === allSprites.player.pos[0] && e.pos[1] === allSprites.player.pos[1]) killPlayer();
    else if (gameBoard[allSprites.player.pos[1]][allSprites.player.pos[0]] > 10) killPlayer();
  });
...
const tileExplosion = (x: number, y: number, i: number) => {
  if (gameBoard[y][x] !== 3 && x > 0 && y > 0 && x < width && y < height) {
    if (gameBoard[y][x] == 4) {
      if (x == powUpPose[0] && y == powUpPose[1]) $(`${powUpPose[0]},${powUpPose[1]}`)!.style.backgroundImage = `url('${elements.powerUrl!}')`
